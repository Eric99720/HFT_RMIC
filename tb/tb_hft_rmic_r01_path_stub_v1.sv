...
module tb_hft_rmic_r01_path_stub_v1;
    ...
    wire risk_reject_valid; reg risk_reject_ready=0; wire [31:0] risk_reject_order_id; wire [1:0] risk_reject_reason_source; wire [7:0] risk_reject_reason_code;
    ...
    task setup;
        ...
        begin
            ...
            g=0; while(!store_init_done) begin @(posedge clk); #1; g=g+1; if(g>1200) fail("store init timeout"); end
            ...
            #1; g=0; while(!cfg_ready) begin @(posedge clk); #1; g=g+1; if(g>200) fail("cfg ready timeout"); end
    ...
    task send_legacy(input [255:0] d);
        ...
        begin
            ...
            while(!legacy_order_ready) begin @(posedge clk); #1; g=g+1; if(g>500) fail("legacy order handshake timeout"); end
    ...
    task send_prebuild(input [255:0] d);
        ...
        begin
            ...
            while(!prebuild_order_accept) begin @(posedge clk); #1; g=g+1; if(g>500) fail("prebuild order handshake timeout"); end
    ...
    task wait_r01(input [31:0] expected_count);
        ...
        begin
            g=0;
            while(encoder_sent_r01_count<expected_count) begin @(posedge clk); #1; g=g+1; if(g>800) fail("R01 timeout"); end
            if(tx_message_type!=0 && tx_message_type!=101) fail("unexpected message type");
        end
    endtask

    task consume_reject;
        begin
            @(negedge clk); risk_reject_ready=1;
            @(posedge clk); @(negedge clk); risk_reject_ready=0;
        end
    endtask

    initial begin
        integer g;
        order_type_allow_mask[`HFT_RMIC_TAIFEX_ORD_LIMIT]=1;
        tif_allow_mask[`HFT_RMIC_TAIFEX_TIF_ROD]=1;
        position_effect_allow_mask[`HFT_RMIC_TAIFEX_POS_OPEN]=1;
        position_effect_allow_mask[`HFT_RMIC_TAIFEX_POS_CLOSE]=1;
        setup();

        send_legacy(mk_order(200,`HFT_RMIC_TMP_SIDE_BUY,`HFT_RMIC_TAIFEX_POS_OPEN));
        wait_r01(1);
        $display("I4_LEGACY_RISK_TO_ENCODER_PASS");

        send_prebuild(mk_order(201,`HFT_RMIC_TMP_SIDE_BUY,`HFT_RMIC_TAIFEX_POS_OPEN));
        wait_r01(2);
        $display("I4_PREBUILD_RISK_TO_ENCODER_PASS");

        // Both producers valid: frozen prebuild priority must prevent legacy handoff.
        @(negedge clk);
        legacy_order_data=mk_order(202,`HFT_RMIC_TMP_SIDE_BUY,`HFT_RMIC_TAIFEX_POS_OPEN);
        prebuild_order_data=mk_order(203,`HFT_RMIC_TMP_SIDE_BUY,`HFT_RMIC_TAIFEX_POS_OPEN);
        legacy_order_valid=1; prebuild_order_valid=1; #1;
        g=0; while(!prebuild_order_accept) begin @(posedge clk); #1; g=g+1; if(g>500) fail("priority prebuild timeout"); end
        if(legacy_order_ready) fail("legacy ready asserted during prebuild priority");
        @(posedge clk); @(negedge clk); prebuild_order_valid=0; #1;
        g=0; while(!legacy_order_ready) begin @(posedge clk); #1; g=g+1; if(g>500) fail("deferred legacy timeout"); end
        @(posedge clk); @(negedge clk); legacy_order_valid=0;
        wait_r01(4);
        $display("I4_PREBUILD_PRIORITY_PASS");

        // Kill-switch reject must never reach the encoder.
        global_kill=1;
        send_prebuild(mk_order(204,`HFT_RMIC_TMP_SIDE_BUY,`HFT_RMIC_TAIFEX_POS_OPEN));
        g=0; while(!risk_reject_valid) begin @(posedge clk); #1; g=g+1; if(g>500) fail("risk reject timeout"); end
        ...
        if(encoder_sent_r01_count!=4) fail("rejected order leaked to R01");
        consume_reject();
        $display("I4_RISK_REJECT_SUPPRESSES_R01_PASS");

        if(recovery_required) fail("unexpected recovery state");
        $display("HFT_RMIC_R01_PATH_STUB_V1_TB_PASS");
        $finish;
    end
endmodule
