...
try {
    ...
    foreach ($phase in $PhasePs) {
        ...
        $sample = [pscustomobject]@{
            ...
            MarketToPrebuildNs = ([double]$m.Groups[6].Value - [double]$m.Groups[2].Value)
            PrebuildToAppNs = ([double]$m.Groups[8].Value - [double]$m.Groups[6].Value)
            PrebuildToStartNs = ([double]$m.Groups[9].Value - [double]$m.Groups[6].Value)
        }
        $samples += $sample
    ...
    $minPrebuildToApp = ($samples.PrebuildToAppNs | Measure-Object -Minimum).Minimum
    $maxPrebuildToApp = ($samples.PrebuildToAppNs | Measure-Object -Maximum).Maximum
    $minPrebuildToStart = ($samples.PrebuildToStartNs | Measure-Object -Minimum).Minimum
    $maxPrebuildToStart = ($samples.PrebuildToStartNs | Measure-Object -Maximum).Maximum
    $maxAppStartNs = ($samples.AppToStartNs | Measure-Object -Maximum).Maximum

    $summary = "I5_LATENCY_SWEEP_SUMMARY samples={0} min_cycles={1} max_cycles={2} min_ns={3:F3} max_ns={4:F3} jitter_ns={5:F3} min_prebuild_to_app_ns={6:F3} max_prebuild_to_app_ns={7:F3} min_prebuild_to_start_ns={8:F3} max_prebuild_to_start_ns={9:F3} max_app_to_start_ns={10:F3}" -f $samples.Count,$minCycles,$maxCycles,$minNs,$maxNs,$jitterNs,$minPrebuildToApp,$maxPrebuildToApp,$minPrebuildToStart,$maxPrebuildToStart,$maxAppStartNs
